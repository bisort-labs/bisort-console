...
class CustomerInfolist
{
        ...
        return Section::make(Localization::translate('common.sections.billing_address'))
            ...
            ->schema([
                TextEntry::make('billing_address_preview')
                    ...
                    ->state(static fn (Customer $record): string => self::formatBillingAddress($record->billing_address))
                    ->placeholder(Localization::translate('common.placeholder'))
                    ->columnSpanFull(),
            ])
        ;
    ...
    private static function getDefaultEntries(): Section
    {
    ...
    private static function formatBillingAddress(mixed $state): string
    {
        if (! is_array($state)) {
        ...
        $parts = array_values(array_filter([
            is_scalar($state['street'] ?? null) ? strval($state['street']) : null,
            is_scalar($state['city'] ?? null) ? strval($state['city']) : null,
            is_scalar($state['state'] ?? null) ? strval($state['state']) : null,
            is_scalar($state['zip'] ?? null) ? strval($state['zip']) : null,
            is_scalar($state['country'] ?? null) ? strval($state['country']) : null,
